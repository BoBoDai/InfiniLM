...
infinicore::Tensor StaticAttentionImpl::forward(const AttentionLayer &layer,
    ...
    // Quantize AFTER permute: the rearrange/permute op does not support
    // quantized dtypes (fp8/int8), so K/V must be permuted in model precision
    // first and then quantized into the cache layout.
